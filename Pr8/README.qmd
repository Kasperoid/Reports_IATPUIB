...
```{r}
#install.packages('duckdb', repos=c('https://duckdb.r-universe.dev', 'https://cloud.r-project.org'), INSTALL_opts = '--no-lock')
```
...
Составляется запрос в базу данных. Необходимо выделить ip-адреса из внутренней сети и найти по каждому ip кол-во байт, с помощью group by и sum. Полученный список отсортировать по убыванию, следовательно, самая первая запись - ответ
